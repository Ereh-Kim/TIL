...
class Hash_Table {
    ...
    Linear_Probing_Function(Index_Value){
        while(this.table[Index_Value] != undefined){
            Index_Value++
        }
        return Index_Value
    }

    Quadratic_Probing_Function(Index_Value){
        let i =1;
        while(this.table[Index_Value] != undefined){
            Index_Value = Index_Value + i*i
            i++
        }
        return Index_Value
    }

    Double_Hashing_Probing_Function(Index_Value){
        let source = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
        let Re_G_key = ''
        while(this.table[Index_Value] != undefined){
        for(let i =0; i< Index_Value; i++){
            Re_G_key += source.charAt(i*source.length)
        }

        let Re_Hash_code = this.Hash_Function(Re_G_key)
        var Re_G_Index = this.convert_To_Index(Re_Hash_code)
        Index_Value = Re_G_Index
        }
        return Index_Value
        }
    ...
    set_Item(key, value){
        ...
        if(Current_Bucket === undefined){
            ...
            // const Linear_probing = this.Linear_Probing_Function(Index_Value)
            // this.table[Linear_probing] = New_Slot

            // const Quadratic_Probing = this.Quadratic_Probing_Function(Index_Value)
            // this.table[Quadratic_Probing] = New_Slot

            const Double_Hashing_Probing = this.Double_Hashing_Probing_Function(Index_Value)
            this.table[Double_Hashing_Probing] = New_Slot
    ...
    /*
    ㄴ> 위 setItem method 는 Opening Addressing 을 차용했다
    ...
    * Linear Probing
    ㄴ> 현재의 index 값으로부터 고정폭만큼 이동하여,
        차례대로 비어 있는 버킷을 탐색하는 방법론

    * Quadratic Proding
    ㄴ> 탐색의 폭을 제곱해서,
        충돌이 일어나지 않는 비어있는 bucket 을 탐색하는 방법론

    * Double Hashing Probing
    ㄴ> Hash( Code ) 값에 대해 한 번더 Hashing 을 거쳐,
        비어있는 bucket 을 탐색하는 방법론 
